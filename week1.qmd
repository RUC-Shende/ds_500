...
- Develop **computational thinking** skills: the ability to harness "thought
processes involved in expressing solutions as computational steps or
...
If you are not familiar with a Unix/Linux **command-line shell**, please take the time to learn about it! It comes pre-installed on Macs and Linux boxes as a `Terminal` application, and on Windows you can either install [WSL 2 (Windows Subsystem for Linux)](https://learn.microsoft.com/en-us/windows/wsl/install) or [Git Bash](https://librarycarpentry.org/lc-shell/index.html#setup).
...
You can install [Anaconda](https://anaconda.com) which comes bundled with Python and all the Python data science-related packages that will be used in this course. It also comes with its own package manager.

::: {.callout-tip}
Consider installing [Miniconda](https://docs.conda.io/projects/miniconda/en/latest/): this is a minimal installer that comes with a package management tool called **`conda`**.
:::

Alternatively, if you want more control over what gets installed, you can install the needed software **individually**:

- [Python: Version 3.11](https://www.python.org/downloads/): check that `pip` or `pip3`, the Python package management utility has been installed. On WSL2, you will need to use the default package manager called `apt` (for Ubuntu Linux, the default distribution installed with WSL) - see the [installation directions here](https://learn.microsoft.com/en-us/windows/python/web-frameworks#install-python-pip-and-venv).

- Once `pip3` installed, you can install **any** Python package from the [PyPi](https://pypi.org) repository. For now, just make sure that `jupyterlab`, `matplotlib`, `numpy`, `scipy` and `pandas` are installed using `pip`.
...
- a browser-based editing environment called a Jupyter notebook that is either local to your laptop (via Jupyter lab) or is hosted through Google's [Colab](https://colab.research.rutgers.com) service.

## Textbook code

The code demonstrated in the textbook is avaialble in a [Github cloud repository](https://github.com/guttag/Intro-to-Computation-and-Programming).
...
- Easily the most popular **high-level** language with native support
for different programming **paradigms** and excellent built-in libraries
...
Consult the [Python Tutor](www.pythontutor.com) website to visualize the **binding** of values to names in a Python code snippet!
...
- Abstraction that **encapsulates** a piece of computation whose outcome (**return value**)
